...
function game() {
  ...
  if (gameConfig.status === 'start') {
    ...
    textSize(40);
    textAlign(CENTER);
    text("da click en el boton:Virus para iniciar", gameConfig.screenX / 2, gameConfig.screenY / 2);
    textSize(40);
  ...
  if (gameConfig.status === 'gameover') {
    ...
    textSize(40);
    textAlign(CENTER);
    text("perdiste", gameConfig.screenX / 2, gameConfig.screenY / 2 + 105);
    textSize(15);
    text("Presiona virus para reiniciar", gameConfig.screenX / 2, gameConfig.screenY / 2 + 135);
    textSize(40);
...
function changeGameStatud(character) {
  if ((iniciar ||keyDown(control.up) || keyDown(control.left) || keyDown(control.right)) && gameConfig.status === "start") {
    initializeCharacterStatus(mario)
    gameConfig.status = "play"
  }
  if (gameConfig.status === "gameover" && (keyDown(control.revive)||iniciar)) {
    gameConfig.status = "start"
  }
} iniciar=false

function virus(){iniciar= true }
...
function manualControl(character) {
  ...
  if (character.live) {
    if (keyDown(control.left) || noseX > 300) {
      character.velocity.x -= gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(-1);
    }
    else if (keyDown(control.right)|| noseX<300) {
      character.velocity.x += gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    }
...
function jumping(character) {
  if ((keyWentDown(control.up) && character.live) || (touchIsDown && character.live)||noseY<125) {
...
function scores(character) {
  ...
  text("marcador: " + round(gameConfig.scores), 20, 40);
  text("vida: " + character.liveNumber, 20, 80);
